...
public class PackageTool : EditorWindow
{
    ...
    static private ChannelType channelType = ChannelType.Ljsd;
    static public string resVersion = "1.0.000";
    ...
    static private bool useHTTPsServer;
    static private string localServerAddress = @":7888/Android/Android/";
    [MenuItem("Tools/Package", false, 0)]
    static void Init() {
        EditorWindow.GetWindow(typeof(PackageTool));
    }
    //[MenuItem("Tools/ConvertXml2Json")]
    //public static void OnConvertXml2Json()
    //{
    //    BuildPlayer.ConvertXml2Json();
    //}
    void OnEnable()
    {
        ...
        useHTTPsServer = PackageUtils.UseHTTPsServer();
    }
    ...
    public static void BuildAssetBundlesForCurrentChannel()
    {
        ...
        Debug.Log("new resversion:" + resVersion);
    ...
    public static bool CheckSymbolsToCancelBuild()
    {
        ...
        if (!string.IsNullOrEmpty(replace))
        {
            int checkClear = EditorUtility.DisplayDialogComplex("Build Symbol Warning",
                string.Format("Now symbols : \n\n{0}\n\nClear all symbols except \"HOTFIX_ENABLE;UNITY_POST_PROCESSING_STACK_V2\" ?", symbols),
                "Yes", "No", "Cancel");
            if (checkClear == 0)
            {
                PlayerSettings.SetScriptingDefineSymbolsForGroup(buildTargetGroup, "HOTFIX_ENABLE;UNITY_POST_PROCESSING_STACK_V2");
            }
            return checkClear == 2;
        }
        return false;
    ...
    public static void BuildAndroidPlayerForCurrentChannel()
    {
        ...
        var start = DateTime.Now;
        BuildPlayer.BuildAndroid(channelType.ToString(), channelType == ChannelType.Ljsd);
    ...
    public static void BuildAndroidPlayerForAllChannels()
    {
        ...
        foreach (var current in (ChannelType[])Enum.GetValues(typeof(ChannelType)))
        {
            BuildPlayer.BuildAndroid(current.ToString(), current == ChannelType.Ljsd);
        }
    ...
    public static void BuildIOSPlayerForCurrentChannel()
    {
        ...
        var start = DateTime.Now;
        BuildPlayer.BuildXCode(channelType.ToString(), channelType == ChannelType.Ljsd);
    ...
    public static void BuildIOSPlayerForAllChannels()
    {
        ...
        foreach (var current in (ChannelType[])Enum.GetValues(typeof(ChannelType)))
        {
            BuildPlayer.BuildXCode(current.ToString(), channelType == ChannelType.Ljsd);
        }
