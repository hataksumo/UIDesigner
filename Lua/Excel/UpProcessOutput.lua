...
local fn_fill_the_sheet = function(v_chaProcessData,v_chaEqpProcessData,v_propSimSheet)
	local row = 3
	local sCardTypeField = {"jlr","shl"}
	for _i,chaData in ipairs(v_chaProcessData) do
		...
		for _lvLoc,levelData in ipairs(chaData.lvs) do
			...
			for pos,coupleData in ipairs(levelData.cardGroup) do
				for _cardTypeFieldLoc=1,#sCardTypeField do
					...
					v_propSimSheet:set_vals("type",row,cardTypeField)
...
local fn_cal_lv_process = function()
	...
	for _iChaType,sChaTypeKey in ipairs(stype) do
		...
		for _i=1,#cha_groups do
			local chaOptData = {}
			table.insert(chaOptProcessData,chaOptData)
			...
			--章节名
			chaOptData.sOptCha = sChaTypeKey.."-".._i
			--章节位置
			...
			for lvLoc,lvnType in ipairs(nTypeCfg) do
				...
				local iOptLvId = (chaIdBase[_iChaType] + _i) * 100 + lvLoc
				--关卡ID
...
local fn_output_excel = function()
	...
	book:open("养成进度.xlsx")
	local sheetDisigner = book:get_sheet("卡牌组导出")
	local lv_process_data = fn_cal_lv_process()
	local fn_cal_eqp_process = dofile"Excel\\UpProcessOutput_eqp"
	local eqp_process_data = fn_cal_eqp_process()
	fn_fill_the_sheet(lv_process_data,eqp_process_data,sheetDisigner)
