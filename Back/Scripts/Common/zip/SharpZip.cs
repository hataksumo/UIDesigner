...
namespace SharpZipTool
{
    public class SharpZip
    {
        ...
        private static Dictionary<string, string> GetAllFileSystemEntities(string source, string topDirectory)
        {
            ...
            int topDirtLength = topDirectory.Length;
            if (Directory.Exists(source))
            {
                ...
                {
                    entitiesDictionary.Add(directory, directory.Substring(topDirtLength));
                }

                string[] files = Directory.GetFiles(source, "*.*", SearchOption.AllDirectories);
                foreach (string file in files)
                {
                    entitiesDictionary.Add(file, file.Substring(topDirtLength));
                }
            }
        ...
        public static bool CompressFile(string path, string zipFilePath,
            string comment = null, string password = null, int compressionLevel = 6, System.Action<float,string> progressInfo = null)
        {
            return CompressFile(new string[] { path }, zipFilePath, comment, password, compressionLevel,progressInfo);
        }
        ...
        public static bool CompressFile(IEnumerable<string> sourceList, string zipFilePath,
            ...
            , System.Action<float, string> progressInfo = null
            )
        {
            bool result = false;
            if (progressInfo != null) progressInfo(0f, "Init");
            ...
            try
            {
                ...
                if (progressInfo != null) progressInfo(0.05f, "Checking Files");
                float max = dictionaryList.Count;
                int count = 0;
                using (ZipOutputStream zipStream = new ZipOutputStream(File.Create(zipFilePath)))
                {
                    ...
                    zipStream.SetLevel(CheckCompressionLevel(compressionLevel));//设置压缩等级
                    foreach (var info in dictionaryList)//从字典取文件添加到压缩文件
                    {
                        count++;
                        if (progressInfo != null) progressInfo(0.05f + (count / max)*0.9f, info.Value );

                        if (File.Exists(info.Key))//判断是文件还是文件夹
                        {
                            FileInfo fileItem = new FileInfo(info.Key);

                            using (FileStream readStream = fileItem.Open(FileMode.Open,
                                FileAccess.Read, FileShare.Read))
                            {
                                ZipEntry zipEntry = new ZipEntry(info.Value);
                ...
                if (progressInfo != null) progressInfo(1f, "Fin!");
                result = true;
            }
            catch (System.Exception ex)
            {
