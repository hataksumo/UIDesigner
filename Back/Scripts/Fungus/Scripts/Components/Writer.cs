...
namespace Fungus
{
    ...
    public class Writer : MonoBehaviour, IDialogInputListener
    {
        ...
        public virtual void Stop()
        {
            ...
            {
                Logger.Log("Stop执行 isWriting " + isWriting);
                exitFlag = true;
                isWriting = false;
                if (isWriting==true)
                {
        ...
        public virtual void CopyParm( bool stopAudio, Action onComplete)
        {
            Writer.stopAudio = stopAudio;
            Writer.onComplete = onComplete;
        }
        public virtual void ResetWriteStates()
        {
                inputFlag = false;
                exitFlag = false;
                isWaitingForInput = false;
                isWriting = false;
            Logger.Log("ResetWriteStates() 执行 isWriting 为" +isWriting);
