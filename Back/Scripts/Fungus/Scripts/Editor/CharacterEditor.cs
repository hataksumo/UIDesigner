...
namespace Fungus.EditorUtils
{
    ...
    public class CharacterEditor : Editor
    {
        ...
        protected SerializedProperty displayCharPortraitProp;
        protected virtual void OnEnable()
        {
            ...
            displayCharPortraitProp = serializedObject.FindProperty("displayPortrait");
        }
        ...
        public override void OnInspectorGUI() 
        {
            ...
            //EditorGUILayout.PropertyField(displayCharPortraitProp);
            t.displayPortrait = EditorGUILayout.ObjectField("Display Portrait",t.displayPortrait,typeof(DialogPortrait),true) as DialogPortrait;
            //char protrait popup menu
            if (t.displayPortrait != null &&
               t.displayPortrait.charLayers != null &&
               t.displayPortrait.charLayers.Length > 0
               )
            {
                if (t.defaultProtraitLayerSetting == null
                    || t.defaultProtraitLayerSetting.Length != t.displayPortrait.charLayers.Length
                    )
                {
                    t.defaultProtraitLayerSetting = new int[t.displayPortrait.charLayers.Length];
                }
                for (int i = 0 ; i < t.displayPortrait.charLayers.Length ; i++)
                {

                    string[] popStr = null;
                    if (t.displayPortrait.charLayers[i].sps != null && t.displayPortrait.charLayers[i].sps.Count > 0)
                    {
                        popStr = new string[t.displayPortrait.charLayers[i].sps.Count];
                        for (int j = 0 ; j < t.displayPortrait.charLayers[i].sps.Count ; j++)
                        {
                            Sprite sp = t.displayPortrait.charLayers[i].sps[j];
                            popStr[j] = ((sp == null) ? "Null" : sp.name);
                        }

                    } else
                    {
                        popStr = new string[] { "None" };
                    }

                    t.defaultProtraitLayerSetting[i] = EditorGUILayout.Popup(
                        new GUIContent(string.Format("    Layer{0} :", i), "")
                        , t.defaultProtraitLayerSetting[i]
                        , popStr);
                }
            }
