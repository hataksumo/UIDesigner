...
-- 使用方式：
-- self.xxx_wrapgroup = self:AddComponent(UIWrapGroup3D, scroll_content_path, UIWrapComponentTypeClass)--初始化
...
local UIWrapGroup3D = BaseClass("UIWrapGroup3D", UIBaseContainer)
...
UIWrapGroup3D.OnInit = OnInit
UIWrapGroup3D.OnCreate = OnCreate
UIWrapGroup3D.SetOnRefresh = SetOnRefresh
UIWrapGroup3D.SetOnClick = SetOnClick
UIWrapGroup3D.SetLength = SetLength
UIWrapGroup3D.AddComponent = AddComponent
UIWrapGroup3D.AddButtonGroup = AddButtonGroup
UIWrapGroup3D.AddButton = AddButton
UIWrapGroup3D.SetOriginal = SetOriginal
UIWrapGroup3D.ResetToBeginning = ResetToBeginning
UIWrapGroup3D.WrapContent = WrapContent
UIWrapGroup3D.OnDestroy = OnDestroy
UIWrapGroup3D.SetGridPositionByIndex = SetGridPositionByIndex
UIWrapGroup3D.GetComponentByIndex = GetComponentByIndex
UIWrapGroup3D.SetChildAlignment = SetChildAlignment
UIWrapGroup3D.RefreshGrid = RefreshGrid
UIWrapGroup3D.GetRealIndex = GetRealIndex
UIWrapGroup3D.SetComponentItemActive = SetComponentItemActive
UIWrapGroup3D.GetLocalPosition = GetLocalPosition
return UIWrapGroup3D