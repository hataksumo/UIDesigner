...
--窗口出现特效
local UICommonMaskComponentManager=require("UI.UICommonMask.UICommonMaskComponentManager");
...
local function OnEnable(self)
	...
	base.OnEnable(self)
	--初始化遮罩窗口、
	--if UIConfig[self.__name].Layer == UILayers.BackgroudLayer then
		--UICommonMaskComponentManager:GetInstance():CreatCommonEffectMask(self,self.__name)
	--end
	self:OnAddListener()
end
...
local function AddCallback(keeper, msg_name, callback)
	assert(callback ~= nil)
	if keeper[msg_name] ~= nil then
		Logger.LogError("重新添加UI事件监听"..msg_name)
	end
...
local function OnDestroy(self)
	--銷毀動銷
	if self.__name ~= nil and UIConfig[self.__name].Layer == UILayers.BackgroudLayer then
		UICommonMaskComponentManager:GetInstance():OnDestroy()
	end
