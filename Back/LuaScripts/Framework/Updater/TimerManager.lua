...
--计时器回调
local function SimpleTimerCall(args)
	local _func = args.func
	local _paras = args.args
	if args.once ~= nil and  args.once and args.timer then
		local self = args.sender
		self.__paras_map[args.timer] = nil
		args.args = false
		args.timer = false
		args.func = false
		args.sender = false
		table.insert(self.__paras_pool, args)
	end
	if _func then
		_func(_paras)
	end
...
local function SimpleTimerStop(self,args)
	if args.timer then
		self.__paras_map[args.timer] = nil
		args.timer:Stop()
		args.args = false
...
--简略版开启时间计时器
local function SimpleTimerArgs(self, delay, func, obj, one_shot, use_frame, unscaled)
	local args = nil
	if table.length(self.__paras_pool) >0  then
		args = table.remove(self.__paras_pool)
	else
		args = {false,false,false,false,false}
	end
	args.timer = self:GetTimer()
	args.args = obj
	args.func = func
	args.sender = self
	args.once = one_shot
	self.__paras_map[args.timer] = args
	args.timer:Init(delay,SimpleTimerCall,args, one_shot, use_frame, unscaled)
	args.timer:Start()
	return args
end
...
local function __delete(self)
	...
	self.__paras_pool = nil
	self.__paras_map = nil
end
...
TimerManager.SimpleTimerArgs = SimpleTimerArgs
TimerManager.SimpleTimerStop = SimpleTimerStop
return TimerManager;