...
local function RecycleGameObject(self, path, inst)
	assert(not IsNull(inst),"path is "..path)
	assert(path ~= nil and type(path) == "string" and #path > 0, "path err : "..path)
	assert(string.endswith(path, ".prefab", true), "GameObject must be prefab : "..path)
...
local function Cleanup(self, includePooledGo)
	Logger.Log("__instCache.Count"..table.count(__instCache))
...
local function GetAllCachePath(self)
	local cache = nil
	if __goPool ~= nil then
		cache = {}
		for i, v in pairs(__goPool) do
...
GameObjectPool.GetAllCachePath = GetAllCachePath
