...
local function __init(self, parent_trans, effect_config)
...
local function LoadResource(self,create_callback)
    if self.config ~= nil then
        -- 回调：用于组合方式
        self.create_callback = create_callback

        if self.config == nil then
            self.gameObject = self.parent_trans.gameObject
            self.transform = self.parent_trans
            GetRenderers(self)
        else
            -- 资源加载
            GameObjectPool:GetInstance():GetGameObjectAsync(self.config.EffectPath, function(go, self)
                if self ~= nil then
                    InitEffect(self, go)
                    self:OnCreate()
...
BaseEffect.LoadResource = LoadResource
return BaseEffect