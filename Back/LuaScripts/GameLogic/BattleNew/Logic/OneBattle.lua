...
local function PveAction(self, Packages)
    ...
    if actionEnd ~= nil and not Packages.actionEnd._is_null then
        ...
        for i, v in ipairs(Packages.actionEnd) do
            ...
            if effectView ~= nil and #effectView > 0 then
                table.insert(endTargets,{blood = bloodView,effect = effectView,teamId = v.teamId,entityId = v.entityId})
            end
        end
    end
...
local function SendPveActionRequest(self,data,type,auto)
    if self.currentRoundState == BattleEnum.BattleRoundState.ROUNDFRONT then
        self.netManager:SendPveActionRequest(self.battlePlayerManager.attackPlayer.Id,data,auto,Bind(self,PveAction))
    end
end
...
local function AddEventListener(self)
    ...
    self.battleMessage:AddListener(BattleEnum.BattleMessage.NOTIFYSIMPLETHREESTATE, SetSimpleThreeState, self)
    self.battleMessage:AddListener(BattleEnum.BattleMessage.NOTIFYSPLAYSTORY, CoTryTriggerStory, self)
end
--移除方法的监听
local function RemoveEventListener(self)
    ...
    self.battleMessage:RemoveListener(BattleEnum.BattleMessage.NOTIFYSIMPLETHREESTATE, SetSimpleThreeState)
    self.battleMessage:RemoveListener(BattleEnum.BattleMessage.NOTIFYSPLAYSTORY, CoTryTriggerStory)
end

local function OnInit(self, battleMessage, myTeam, myGuard, battlePackages, netManager, storyTrigger,battleEnemyTeam)
    self.round = 1
    ...
    self.battlePlayerManager:OnInit(self, myTeam, myGuard, battlePackages,battleEnemyTeam)
    local uid = ClientData:GetInstance():GetServerUserId()
    if BattleFieldManager:GetInstance().battleType==BattleEnum.BattleType.SOLO_ARENA then
        self.simpleThreeBeat =  CS.UnityEngine.PlayerPrefs.GetInt("simple_three_beat_"..uid.."_solo_arena", 0)
        self.auto_battle = CS.UnityEngine.PlayerPrefs.GetInt("auto_battle_"..uid.."_solo_arena", 0)
        self.speed_state = CS.UnityEngine.PlayerPrefs.GetInt("speed_state_"..uid.."_solo_arena", 0)
    else
        self.simpleThreeBeat =  CS.UnityEngine.PlayerPrefs.GetInt("simple_three_beat_"..uid, 0)
        self.auto_battle = CS.UnityEngine.PlayerPrefs.GetInt("auto_battle_"..uid, 0)
        self.speed_state = CS.UnityEngine.PlayerPrefs.GetInt("speed_state_"..uid, 0)
    end
end
...
local function ChangeRoundState(self,state)
    ...
    if self.currentRoundState == BattleEnum.BattleRoundState.ROUNDFRONT then
        CoTryTriggerStory(self,3,nil,function()
            self.netManager:SendConvertCrytalsRequest(self.battlePlayerManager.attackPlayer.Id,Bind(self,ConvertCrytals))
        end)
...
OneBattle.SendPveActionRequest = SendPveActionRequest
OneBattle.BeginBattle = BeginBattle
OneBattle.ChangeRoundState = ChangeRoundState
OneBattle.CoTryTriggerStory = CoTryTriggerStory
return OneBattle