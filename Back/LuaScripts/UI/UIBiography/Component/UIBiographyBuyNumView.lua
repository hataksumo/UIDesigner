...
---
local UIBiographyBuyNumView = BaseClass("UIBiographyBuyNumView", UIWrapComponent)
...
local staticGloabalData = DataUtil.GetData("global")
...
local function BuyItem(self)
    ...
    if haveCoin>=self.finalPrice then
        ...
        BiographyData:GetInstance():OnBuyBiographyTimes(self.buyNum, self.finalPrice);
    else
...
local function GetCurCoinCanByNum(self,_haveCoin)
    ...
    local curPrice=0;
    for i = 1, #self.priceGroup do
        if i>initIndex then
            curPrice=self.priceGroup[curIndex]+curPrice;
    ...
    if curPrice<_haveCoin then
        finalNum=math.floor((_haveCoin-curPrice)/self.priceGroup[#self.priceGroup])+finalNum;
    end
...
local function GetFinalNeedCoinByNum(self,buyNum)
    ...
    for i = 1, #self.priceGroup do
        if i==todayBuyCount then
            if curIndex<buyNum then
                finalPrice=self.priceGroup[todayBuyCount]+finalPrice;
    ...
    finalPrice=finalPrice+(buyNum-curIndex)*self.priceGroup[#self.priceGroup];
    return finalPrice;
end
...
local function GetNextNeedCoinBuyNum(self,buyNum)
    local nextPrice = 0;
    local priceNum = #self.priceGroup;
    if self.todayBuyCount + buyNum + 1 >= priceNum then
        nextPrice = self.priceGroup[priceNum]
    else
        nextPrice = self.priceGroup[self.todayBuyCount + buyNum + 1]
    end
    return nextPrice
end
...
local function OnCreate(self)
    ...
    self.priceGroup = staticGloabalData[179].valueNA
end
...
local function SetData(self)
    ...
    self.buyItemIcon:SetSpriteName(staticItemData[CoinDefine.Diamond].Icon);
    ...
    self.curleftCount = #self.priceGroup - BiographyData:GetInstance().haveBuyNum;
    if self.curleftCount <= 0 then
        UISpecial:GetInstance():UITipText(DataUtil.GetClientText(100138))
        return
    end
    SetActive(self, true)
    --设置限制次数文本显示
    self.desText:SetText("当前剩余总次数："..math.floor(self.curleftCount));
    self.isCheckSliderValueChange=true;
    self.todayBuyCount = BiographyData:GetInstance().haveBuyNum;
    local haveCoin=BackpackData:GetInstance():GetItemNumById(CoinDefine.Diamond);
    local canBuyNum=GetCurCoinCanByNum(self,haveCoin);
    self.leftCount=canBuyNum==0 and 1 or math.min(self.curleftCount,canBuyNum);
    self.limit_text:SetText(string.format(DataUtil.GetClientText(100010),DataUtil.GetClientText(100007),math.modf(self.leftCount)));
    --设置价格
    ...
    self.nextItemIcon:SetSpriteName(staticItemData[CoinDefine.Diamond].Icon);
    self.buyNum=1;
...
UIBiographyBuyNumView.OnAddListener = OnAddListener
UIBiographyBuyNumView.OnRemoveListener = OnRemoveListener
UIBiographyBuyNumView.OnCreate = OnCreate;
UIBiographyBuyNumView.OnEnable=OnEnable;
UIBiographyBuyNumView.SetData =SetData
UIBiographyBuyNumView.SetActive = SetActive
return UIBiographyBuyNumView;