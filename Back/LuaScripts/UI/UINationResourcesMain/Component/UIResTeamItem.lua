local UIResTeamItem = BaseClass("UIResTeamItem", UIWrapComponent)
...
local function OnCreate(self)
    ...
    self.allianceImage = self:AddComponent(UIImage, "InfoPanel/allianceImage")
...
local function OnRefresh(self, real_index)
    ...
    if self.view.curSelectTeamType == self.view.TeamType.battle then
        itemData = self.view.model.teamBattleList[real_index]
    elseif self.view.curSelectTeamType == self.view.TeamType.defend then
        itemData = self.view.model.teamDefendList[real_index]
    end
    ...
    if isNilOrEmpty(itemData.ArmyGroup) then
        self.allianceImage:SetEnabled(false)
    else
        self.allianceImage:SetEnabled(true)
    end
    ...
    elseif itemData.State == NationDefine.NATION_TEAM_BEHAVIOR_STATE.COLLECT then
        self.state_text:SetText(DataUtil.GetClientText(200067))
    elseif itemData.State == NationDefine.NATION_TEAM_BEHAVIOR_STATE.WAITING then
        self.state_text:SetText(DataUtil.GetClientText(200068))
    elseif itemData.State == NationDefine.NATION_TEAM_BEHAVIOR_STATE.EMPTY then
    ...
    local remainTime = itemData.Time - math.modf(TimeSyncManager:GetInstance():GetSimulationJavaMS() / 1000)
    if remainTime > 0 then
        self.time_text:SetText(os.date("%M:%S", remainTime))
...
UIResTeamItem.OnCreate = OnCreate
UIResTeamItem.OnRefresh = OnRefresh
UIResTeamItem.OnDisable = OnDisable

return UIResTeamItem